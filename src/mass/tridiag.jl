struct TriDiagMass <: AbstractMass{2}
    R::Int
    C::Int
    N::Int
...
function Base.size(m::TriDiagMass) return (m.R, m.C) end 
...
Base.rand(m::TriDiagMass, buffer) = begin
    buf = pop!(buffer)
    buf .= reshape(m.L \ randn(length(m)), m.C, m.R)' |> collect
    return buf
end
